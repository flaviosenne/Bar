...
controller.listar = async (req, res) => {
   ...
   if(Object.keys(req.query).length > 0) {
      ...
      try {
         ...
         const lista = await Promotor.find()
...
controller.obterUm = async (req, res) => {
   try {
      ...
      const obj = await Promotor.findById(id)
...
controller.atualizar = async (req, res) => {
   try {
      ...
      const obj = await Promotor.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
   try {
      const id = req.body._id
      const obj = await Promotor.findByIdAndDelete(id)
      if(obj) {
         res.status(204).end()
      }
      else {
...
async function busca (req, res) {
   ...
   try{
      let lista = await Promotor.find(criterio)
      res.send(lista)
   }
   catch(erro) {
      console.log(erro)
