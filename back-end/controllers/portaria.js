...
controller.novo = async (req, res) => {
   try {
      await Portaria.create(req.body)
...
controller.listar = async (req, res) => {
   ...
   if(Object.keys(req.query).length > 0) {
      ...
      try {
         ...
         const lista = await Portaria.find().populate(
            { path: 'promotor', select: 'nome telefone'}
       )
...
controller.obterUm = async (req, res) => {
   try {
      ...
      const obj = await Portaria.findById(id)
...
controller.atualizar = async (req, res) => {
   try {
      ...
      const obj = await Portaria.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
   try {
      const id = req.body._id
      const obj = await Portaria.findByIdAndDelete(id)
      if(obj) {
         res.status(204).end()
      }
      else {
...
async function busca (req, res) {
   ...
   try{
      let lista = await Portaria.find(criterio)
      res.send(lista)
   }
   catch(erro) {
      console.log(erro)
